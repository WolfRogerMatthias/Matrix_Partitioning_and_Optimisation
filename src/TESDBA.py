"""
This file is for the Evaluation of the Bucket Algorithm in Timing and Accuracy
...
for i in range(number_of_runs):
    ...
    for matrix_size in matrix_sizes:
        ...
        bucket_run.append(bucket_end - bucket_start)

    lsa_timings.append(lsa_run)
    bucket_timings.append(bucket_run)
    end = time.time()
...
bucket_min, bucket_max = trimmed_min_max(bucket_arr)
...
plt.plot(matrix_sizes, bucket_mean, label='Bucket (Average)', color=colors['bucket'], linewidth=2)
plt.fill_between(matrix_sizes, bucket_min, bucket_max, color=colors['bucket'], alpha=0.2,
                 label='Bucket Range')
plt.legend()
plt.xticks(matrix_sizes)
plt.xlabel("Matrix Size (n x n)")
plt.ylabel("Execution Time (s)")
plt.title("LSA vs Bucket - Timing Comparison")
...
plt.tight_layout()
plt.savefig(f'./png/{timestamp}_lsa_vs_bucket_timing.png')
plt.show()

...
for matrix_size in matrix_sizes:
    ...
    direct_mappings.append(direct_mapping)
...
def evaluation_metrics(cost_matrices, optimal_mappings, approx_mappings):
    ...
    for i in range(len(cost_matrices)):
        ...
        for j in range(len(optimal_mapping)):
            ...
            app_ele = [matrix[row, col] for row, col in approx_mapping[j].items()]
            opt_sum = sum(opt_ele)
            app_sum = sum(app_ele)
            ...
            for k in range(n):
                count += 1 if (y_opt_map[k] == k and x_opt_map[k] == x_app_map[k]) else 0
...
data = evaluation_metrics(matrices, lsa_mappings, direct_mappings)
...
plt.plot(matrix_sizes, mean_acc, label='Mean Assignment Accuracy', color=colors['direct'])
plt.fill_between(matrix_sizes, mean_acc - std_acc, mean_acc + std_acc, alpha=0.2, color=colors['direct'])
plt.xlabel('Matrix Size')
plt.ylabel('Assignment Accuracy')
plt.xticks(matrix_sizes)
plt.ylim(0, 1)
plt.xlim(matrix_sizes[0], matrix_sizes[-1])
plt.title('Assignment Accuracy of Direct Algorithm')
plt.grid(True)
plt.legend()
plt.savefig(f'./png/{timestamp}_acc_direct.png')
plt.show()

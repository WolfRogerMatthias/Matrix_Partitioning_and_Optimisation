...
for matrix_size in matrix_sizes:
    ...
    divider_mappings.append(divider_mapping)

    end = time.time()
    print(f'{matrix_size}: {end - start:4.2}')
...
def evaluation_metrics(cost_matrices, optimal_mappings, approx_mappings):
    ...
    for i in range(len(cost_matrices)):
        ...
        for j in range(len(optimal_mapping)):
            ...
            y_app_map = approx_mapping[j][0]
            x_app_map = approx_mapping[j][1]
            matrix = matrice[j]
            opt_ele = matrix[y_opt_map, x_opt_map]
            app_ele = matrix[y_app_map, x_app_map]
            opt_sum = sum(opt_ele)
            app_sum = sum(app_ele)
            ...
            n_gap_r = []
            for k in range(n):
                count += 1 if (y_opt_map[k] == y_app_map[k] and x_opt_map[k] == x_app_map[k]) else 0
...
data = evaluation_metrics(matrices, lsa_mappings, divider_mappings)
# --- Assignment Accuracy ---
mean_acc = np.mean(data['tp'], axis=1)
std_acc = np.std(data['tp'], axis=1)

plt.figure(figsize=(8, 6))
plt.plot(matrix_sizes, mean_acc, label='Mean Assignment Accuracy', color=colors['divider'])
plt.fill_between(matrix_sizes, mean_acc - std_acc, mean_acc + std_acc, alpha=0.2, color=colors['divider'])
plt.xlabel('Matrix Size')
plt.ylabel('Assignment Accuracy')
plt.xticks(matrix_sizes)
plt.ylim(0, 1)
plt.xlim(matrix_sizes[0], matrix_sizes[-1])
plt.title('Assignment Accuracy of Matrix Divider Algorithm')
plt.grid(True)
plt.legend()
